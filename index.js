...
const imagenes = {
  // 0: {
  //   img: "https://youtu.be/B29M9JG8Q9o",
  //   title: "Nuestra Historia",
  //   description:
  //     "CIB - Corporación para investigaciones biológicas",
  //   date: "1970",
  // },
  1: {
  ...
  13: {
    ...
    title: "2003",
    description:
      "Creamos oficialmente la Unidad de Biodiversidad.",
    date: "2003",
  },
  14: {
  ...
  19: {
    ...
    title: "2017",
    description:
      "La Universidad de Santander se vincula como socio de la CIB.",
    date: "2017",
  },
  20: {
...
// esta funcion recibe el indice de la foto para cambiar la informacion
function cambiarDate(indice) {
  foto.setAttribute("src", imagenes[indice]["img"]);
  foto.setAttribute("id", indice);

  description.textContent = imagenes[indice]["description"];
  title.textContent = imagenes[indice]["title"];
  date.textContent = imagenes[indice]["date"];
}

window.indice; //variable global en el codigo

// esta funcion recibe lado y indice, cambia la informacion dependiendo la flecha pulsada

function pasar(lado, indice) {
  let num = parseInt(indice);

  if ((num === 0 && lado === "izq") || (num === 1 && lado === "izq"))
    indice = 1;
  if (num > 1 && lado === "izq") indice--;

  if (num >= 1 && lado === "der" && num < Object.values(imagenes).length)
    indice++;
  if (num === 0 && lado === "der") indice = 2;

  cambiarDate(indice);
}

// recupera el evento click en la flecha izquierda
document.querySelector(".izq").addEventListener("click", (e) => {
  e.preventDefault();
  pasar("izq", foto.id);
});

// recupera el evento click en la flecha derecha
document.querySelector(".der").addEventListener("click", (e) => {
  e.preventDefault();
  pasar("der", foto.id);
});

window.contador = 1; // contador global para los indices

// iteratua entre todas las imagenes y crea la miniatura de las mismas
Object.values(imagenes).forEach((item) => {
  let nodo = `
      <img 
      class="miniatura"
      src="${item["img"]}" 
  ...
  containerImages.innerHTML += nodo; // lo incrustamos en el html
  contador++; // sumamos 1 para el id de las imagenes
});

// añade el envento click en cada miniatura de las imagenes
document.querySelectorAll(".miniatura").forEach((item) => {
  item.addEventListener("click", (e) => {
    e.preventDefault();

    cambiarDate(item.getAttribute("id"));
    val = item.getAttribute("id");
  });
});
